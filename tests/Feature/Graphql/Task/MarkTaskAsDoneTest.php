...
function markTaskAsDone(array $data, ?string $token = null)
{
    return postGraphql(
        data: [
...
test('unauthenticated user can not mark task as done', function () {
    markTaskAsDone([
        'id' => 1,
    ])
...
test('can not mark another user task as done', function () {
...
test('can mark task as done', function () {
    /** @var \App\Models\User $user */
    $user = User::factory()->create();
    $token = auth('api')->fromUser($user);
